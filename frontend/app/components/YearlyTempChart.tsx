...
import { CITY_TEMPS, MONTHS_AR, ClimateKey } from "@/app/lib/cityWeather";

export default function YearlyTempChart({ climate, title }: { climate: ClimateKey; title?: string }) {
  const data = CITY_TEMPS[climate];
  if (!data) return null;
  ...
  return (
    <div style={{ background: "white", borderRadius: "16px", padding: "16px", boxShadow: "0 8px 24px rgba(0,0,0,0.06)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px", marginBottom: "8px" }}>
        <strong style={{ color: "var(--navy)" }}>🌡️ متوسط الحرارة على مدار السنة{title ? ` — ${title}` : ""}</strong>
        <span style={{ fontSize: "13px" }}><span style={{ color: "#e67e22" }}>● الهواء</span> &nbsp; <span style={{ color: "#2e75b6" }}>● الماء</span></span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }} role="img" aria-label={`درجات الحرارة ${title || ""}`}>
        {gridVals.map((g) => (
          <g key={g}>
        ...
        {MONTHS_AR.map((m, i) => <text key={m} x={x(i)} y={H - 22} fontSize={12} fill="#475569" textAnchor="middle">{m}</text>)}
      </svg>
