  ...
  return (
    <main style={{ background: "var(--background)" }}>
      <section style={{ background: "linear-gradient(135deg, #08233e 0%, #0d2c54 45%, #2e75b6 100%)", color: "white", padding: "90px 20px", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "54px", lineHeight: 1.3, marginBottom: "20px", maxWidth: "820px", marginInline: "auto" }}>{hero.title}</h1>
        ...
        <div style={{ display: "flex", gap: "15px", justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <Link href={hero.secondaryHref || "/logbook"} style={{ background: "transparent", color: "white", border: "1px solid white", padding: "14px 30px", borderRadius: "10px", fontSize: "17px" }}>{hero.secondaryLabel}</Link>
        </div>
      </section>

      <section style={{ padding: "80px 20px", background: "white" }}>
        <h2 style={{ textAlign: "center", fontSize: "36px", color: "var(--navy)", marginBottom: "14px" }}>{c.tracksTitle}</h2>
        <p style={{ textAlign: "center", color: "#666", marginBottom: "50px", fontSize: "18px" }}>{c.tracksSubtitle}</p>
        <div style={{ maxWidth: "1100px", margin: "auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: "26px" }}>
          {tracks.map((t: any, i: number) => (
            <div key={i} style={{ background: "var(--background)", borderRadius: "18px", padding: "34px 28px", borderTop: "4px solid var(--gold)" }}>
              <div style={{ fontSize: "44px", marginBottom: "14px" }}>{t.icon}</div>
              <h3 style={{ color: "var(--navy)", fontSize: "23px", marginBottom: "12px" }}>{t.title}</h3>
              <p style={{ color: "#555", lineHeight: 1.8 }}>{t.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "80px 20px", background: "var(--background)" }}>
        <h2 style={{ textAlign: "center", fontSize: "36px", color: "var(--navy)", marginBottom: "50px" }}>{c.perksTitle}</h2>
        <div style={{ maxWidth: "1100px", margin: "auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "26px" }}>
          {perks.map((p: any, i: number) => (
            <div key={i} style={{ background: "white", borderRadius: "18px", padding: "30px 26px", boxShadow: "0 10px 30px rgba(0,0,0,0.06)", textAlign: "center" }}>
              <div style={{ fontSize: "40px", marginBottom: "14px" }}>{p.icon}</div>
              <h3 style={{ color: "var(--navy)", fontSize: "20px", marginBottom: "10px" }}>{p.title}</h3>
